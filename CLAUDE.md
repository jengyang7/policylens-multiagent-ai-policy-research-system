...
Verified research routes compact → `evidence_audit` → optional bounded gap research → synthesize. The audit checks planned-subtask coverage, source authority/diversity, contradictions, and material missing evidence; it emits ≤3 follow-up questions for one `Send` fan-out (`gap_subagent`), then `recompact` folds the added findings into the summary. There is no unrestricted researcher reflection loop. `POST /research` with `debate: true` inserts `debate_advocate` ⇄ `debate_skeptic` (2 rounds) → `judge_debate` before that same shared audit. The verdict is streamed as `debate_verdict` and rendered as an informational card. Debate turns stream as `debate_turn` events and checkpoint individually; debate runs also use `stream_mode="messages"` for token-by-token `debate_token` events. The audit streams `evidence_auditing` / `evidence_audit`; its optional follow-up round streams `subtask_done {stage:"gap"}`. Debate remains optional and experimental; `verify_citations` is unaffected.
