...
Runs the full Research Library pipeline for one question and evaluates it:
  ...
  4. Answer Faithfulness — % of answer claims supported by the chunks.

No ground truth required — both metrics use LLM-as-judge.
"""
...
async def evaluate_rag(
    ...
    # Evaluate both metrics concurrently
    import asyncio
    (precision_verdicts, context_precision, precision_usage), \
    (faithfulness_verdicts, answer_faithfulness, faithfulness_usage) = await asyncio.gather(
        run_context_precision(question, chunks, eval_model),
        run_rag_faithfulness(answer, chunks, eval_model),
    )

    all_usage: list[TokenUsage] = [*precision_usage, *faithfulness_usage]
